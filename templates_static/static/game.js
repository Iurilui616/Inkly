...
const gameState = {
    playerId: window.PLAYER_ID || new URLSearchParams(window.location.search).get('player_id'),
    sessionId: null,
    prompt: null,
    timeLimit: 20,
    timeLeft: 20,
...
document.addEventListener('DOMContentLoaded', () => {
    log('playerId: ' + gameState.playerId);
    initCanvas();
    initBrushes();
    initColors();
    initRGBSliders();
...
function initColors() {
    ...
    // Limpa container para evitar duplicação se recarregar
    container.innerHTML = '';

    colors.forEach(color => {
        const btn = document.createElement('button');
        btn.className = 'w-8 h-8 rounded-lg border-2 border-gray-300 hover:scale-110 transition-transform focus:outline-none';
        btn.style.backgroundColor = color;
        btn.onclick = () => {
            selectColor(color);
            // Remove destaque de todos
            Array.from(container.children).forEach(b => b.classList.remove('ring-2', 'ring-offset-2', 'ring-gray-600'));
            // Adiciona destaque neste
...
// Função auxiliar para converter Hex (#RRGGBB) para valores numéricos RGB
...
// Substituir a função selectColor
